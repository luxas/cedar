...
pub enum Residual {
    ...
    /// TODO: Do we want to make sure the error that is got from TPE is the same as from normal eval? I guess
    Error {
        err: EvaluationError,
        /// Type of the value
        ty: Type,
...
impl TryFrom<&Expr<Option<Type>>> for Residual {
    ...
    fn try_from(expr: &Expr<Option<Type>>) -> std::result::Result<Self, ExprToResidualError> {
        ...
        Ok(Residual::Partial { kind, ty, source_loc: expr.source_loc().cloned() })
    }
}
    ...
    fn test_can_error_assuming_well_formed() {
        ...
        assert_eq!(
            Residual::Error{
                ...
                err: EvaluationError::failed_extension_function_application("foo".parse().unwrap(), "failed".into(), None, None)
            }
            .can_error_assuming_well_formed(),
            true
        );
