...
impl Evaluator<'_> {
    ...
    pub fn interpret(&self, r: &Residual) -> Residual {
        let (kind, source_loc) = match r {
            ...
            Residual::Partial { kind, source_loc, .. } => (kind, source_loc),
        };
        // let ty = r.ty().clone();
        ...
        let mk_concrete = |v: Value| Residual::Concrete { value: v, ty: r.ty().clone() };
        ...
        match kind {
                ...
                match (&arg1, &arg2) {
                    (
                        ...
                        BinaryOp::Add | BinaryOp::Sub | BinaryOp::Mul => {
                            match crate::evaluator::binary_arith(*op, v1.clone(), v2.clone(), source_loc.as_ref()) {
                                Ok(v) => mk_concrete(v),
                                Err(e) => mk_error(e),
                            }
                        }
                                ...
                                Ok(uid1) => {
                                    if let Ok(uid2) = v2.get_as_entity() {
                                        ...
                                        mk_error(EvaluationError::type_error(
                                            nonempty![ast::Type::Set, ast::Type::entity_type(crate::evaluator::names::ANY_ENTITY_TYPE.clone())],
                                            &v2,
                                        ))
                                    }
                                }
                        ...
                        BinaryOp::ContainsAll => match (v1.get_as_set(), v2.get_as_set()) {
                            (Ok(arg1_set), Ok(arg2_set)) => mk_concrete(arg2_set.is_subset(arg1_set).into()),
                        ...
                        BinaryOp::ContainsAny => match (v1.get_as_set(), v2.get_as_set()) {
                            (Ok(arg1_set), Ok(arg2_set)) => mk_concrete((!arg1_set.is_disjoint(arg2_set)).into()),
                            // if both arg1 and arg2 errors, propagate arg1's error, as in the concrete evaluator
                            (Err(e), _) => mk_error(e),
                            (Ok(_), Err(e)) => mk_error(e),
                        },
                    ...
                    (Residual::Error { err, ty }, _) => Residual::Error { err: err.clone(), ty: ty.clone() },
                    (_, Residual::Error { err, ty }) => Residual::Error { err: err.clone(), ty: ty.clone() },
                    ...
                    .collect::<std::result::Result<Vec<_>, _>>()
                {
                    ...
                    match self.extensions.func(fn_name) {
                        Ok(ext_fn) => match ext_fn.call(&vals) {
                            ...
                            Ok(PartialValue::Residual(_)) => mk_error(EvaluationError::failed_extension_function_application(
                                fn_name.clone(), "Extension function returned PartialValue::Residual".into(), source_loc.clone(), None)),
                            Err(e) => mk_error(e),
                        },
                        Err(e) => mk_error(e.into()),
                    }
                } else if let Some((err, ty)) = args.iter().flat_map(|r| match r {
                    Residual::Error { err, ty } => Some((err, ty)),
                    _ => None,
                }).next() {
                    // return the first error that was encountered when processing arguments, like normal eval
                    Residual::Error { err: err.clone(), ty: ty.clone() }
                } else {
            ...
            ResidualKind::GetAttr { expr, attr } => {
                ...
                match expr {
                            ...
                            if let Some(attrs) = &entity.attrs {
                                if let Some(val) = attrs.get(attr) {
                                    ...
                                    return mk_error(EvaluationError::entity_attr_does_not_exist(
                                        ...
                                        attr.clone(),
                                        attrs.keys(),
                                        entity.tags.as_ref().map(|tags| tags.contains_key(attr)).unwrap_or(false),
                                        attrs.len(),
                                        source_loc.clone(),
                                    ));
                                }
                    ...
                    Residual::Concrete { value, .. } => mk_error(EvaluationError::type_error(
                        nonempty![
                            ast::Type::Record,
                            ast::Type::entity_type(crate::evaluator::names::ANY_ENTITY_TYPE.clone()),
                        ],
                        &value,
                    )),
            ...
            ResidualKind::Set(es) => {
                ...
                } else if let Some((err, ty)) = rs.iter().flat_map(|r| match r {
                    Residual::Error { err, ty } => Some((err, ty)),
                    _ => None,
                }).next() {
            ...
            ResidualKind::Record(m) => {
                    ...
                    .collect::<std::result::Result<BTreeMap<_, _>, ()>>()
                {
                    ...
                    for (a, r) in record {
                        match r {
                            Residual::Error { err, ty } => {
                                return Residual::Error { err, ty }
                            }
                            _ => {
                                m.insert(a, r);
                            }
...
mod tests {
    ...
    fn test_binary_app() {
        ...
        assert_eq!(
            match eval.interpret(&parse_residual(r#"1 == 1 && decimal("skdfjk") < decimal("0.23") && 9223372036854775807 + 1 == 1"#)) {
